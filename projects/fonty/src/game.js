 ...
 * Fonty playable.
 *
 * Flow:  demo (scripted: type -> styles -> copy -> story -> paste)
 ...
 *
 * The demo drives the very same DOM the player uses, so it doubles as a tutorial.
...
(function () {
  ...
  function brand() {
    ...
    (cfg.endBullets || []).forEach(function (b) {
      var li = document.createElement('li'); li.textContent = b; $('bullets').appendChild(li);
    });
    placeholder.textContent = cfg.placeholder || '';
    turn.textContent = cfg.yourTurnLabel || 'Your turn!';
  }
  ...
  function runDemo() {
    ...
    Array.from(text).forEach(function (ch) {
      p = p.then(function () { press(ch); return wait(cfg.demoTypeMs || 62); });
    });
